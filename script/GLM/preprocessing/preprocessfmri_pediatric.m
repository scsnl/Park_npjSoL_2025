...
% Yuan Zhang, 2025-03-13, modified for using pediatric template for
% preprocessing & cleaned up the script

spm_version             = 'spm12';
software_path           = '/ ㅇtoolboxes/';
spm_path                = fullfile(software_path, spm_version);
spmpreprocscript_path   = '/scsnlscripts/brainImaging/mri/fmri/preprocessing/spm12/';
...
for isubj = 1:numsubj
  ...
  if ismember('c', wholepipeline)
     ...
     if ismember('g',wholepipeline)
          ...
          if isempty(dir(fullfile(SPGRdir, ['seg' ,'_pediatric_', spm_version],['y_',SPGRfilename,'.nii'])))
             fprintf('>>>>> the deformation of user specified anatomical image is %s \n',fullfile(SPGRdir, ['seg' ,'_pediatric_', spm_version], ['y_',SPGRfilename,'.nii']));
             error('Error: the deformation of user specified anatomical image is not found, use preprocessmri.m');
          else
             unix(sprintf('gunzip -fq %s',fullfile(SPGRdir, ['seg' ,'_pediatric_', spm_version],[SPGRfilename,'.nii'])));
             SPGRfile_file = fullfile(SPGRdir, ['seg' ,'_pediatric_', spm_version],[SPGRfilename,'.nii']);
          end
  ...
  for irun = 1:numrun
    ...
    for cnt = 1:nstep
       ...
       switch p
            ...
            case 'c' % co-registration between subject's structural and functional images
                [inputimg_file, selecterr] = preprocessfmri_selectfiles(temp_dir, prevprefix, data_type);
                if selecterr == 1
                     error('Error: no scans selected');
                end
            ...
            case 'w'
                ...
                preprocessfmri_normalize_pediatric(wholepipeline, currentdir, template_path, boundingboxdim, [pipeline, inputimgprefix], inputimg_file, meanimg_file, temp_dir, SPGRfile_file, spm_path);
            ...
            case 'g'
                ...
                listfile_file = dir(fullfile(SPGRdir, ['seg' ,'_pediatric_', spm_version],[ 'y_',SPGRfilename,'.nii']));
                if isempty(listfile_file)
                    error('Error: no segmentation has been done, use preprocessmri.m');
                else
                    disp('------------------------------------------------------------------');
                    fprintf('Segmentation file locates at: \n %s \n \n ',fullfile(SPGRdir, ['seg' '_pediatric_' spm_version],listfile_file(1).name));            
                    %%% --- copy y_*.nii to tmp file
                    unix(sprintf('cp -af %s %s',fullfile(SPGRdir, ['seg' '_pediatric_' spm_version], listfile_file(1).name),temp_dir));
